...
class FrameMatrixDataTypeTest(sparktk_test.SparkTKTestCase):
    ...
    def test_frame_create_row_count(self):
        ...
        frame = self.context.frame.create(self.dataset, schema=self.schema)
    ...
    @unittest.skip(
        "sparktk: schema inference between matrix and vector is ambiguous")
    ...
    def test_schema_with_invalid_schema_all_columns_same_datatype(self):
        """Test with validate_schema=True, invalid schema, columns same type"""
    ...
    def test_add_columns(self):
        ...
        expected_result = [[numpy.array(item[1]).shape[0]] for item in
                           self.dataset]
    ...
    def test_filter(self):
        ...
        # Get number of rows in each matrix from shape of
        ...
        # Get expected result by converting the actual dataset to ndarray
        # and testing the same condition
        filtered_result_matrix = list(
            ifilter(lambda i: numpy.array(i[1]).shape[0] == 2, self.dataset))
        expected_result_matrix = list(
            imap(lambda row: [numpy.array(row[1])], filtered_result_matrix))
        expected_result = len(expected_result_matrix)

        self.assertEqual(obtained_result, expected_result)
        numpy.testing.assert_array_equal(
            obtained_result_matrix, expected_result_matrix)

    def test_convert_matrix_col_to_vector(self):
        """ Convert a matrix column to vector using add_columns"""
        frame = self.context.frame.create(self.dataset, self.schema)

        # Filter the rows which have more than 2 rows as the final vector
        # construction can be for only 2 values as vector needs the length
        # to be defined
        frame.filter(lambda row: row["C1"].shape[0] == 2)

        # Add first column of each matrix as a new column with vector data type
        frame.add_columns(
            lambda row: row["C1"][:, 0], ('first_column', vector(2)))
        obtained_result = frame.take(10, columns='first_column')

        # Convert the first 2 elements of the dataset to numpy array and
        # get the fist column
        expected_result = [[numpy.array(item[1])[:, 0]] for item in
                           self.dataset[:2]]
    ...
    def test_matrix_svd(self):
        ...
        for i, row in results.iterrows():
            ...
            numpy.testing.assert_almost_equal(
                actual_U, U, decimal=4, err_msg="U incorrect")
            numpy.testing.assert_almost_equal(
                actual_V, V, decimal=4, err_msg="V incorrect")
            numpy.testing.assert_almost_equal(
