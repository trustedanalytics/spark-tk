...
class ArimaxTest(sparktk_test.SparkTKTestCase):
    ...
    def setUp(self):
        ...
        # Get last 10 rows
        self.actual_data = self.frame.copy(where=lambda row: row.Int > 99)
    ...
    def test_arima_scoring(self):
        ...
        model_path = output.export_to_mar(self.get_export_file(
            self.get_name("arima")))
    ...
    def test_arx_scoring(self):
        ...
        timeseries_column = self.actual_data.take(
            n=self.actual_data.count(), columns=self.ts_column)
        y = [item for sublist in timeseries_column for item in sublist]

        x_columns = self.actual_data.take(
            n=self.actual_data.count(), columns=self.x_columns)
        ...
        model_path = output.export_to_mar(
            self.get_export_file(self.get_name("arx")))
    ...
    def test_max_scoring(self):
        ...
        output = self.context.models.timeseries.max.train(
            self.train_frame, self.ts_column, self.x_columns, 1, 0)
        ...
        expected_score = [item for sublist in predict_data for item in sublist]
        model_path = output.export_to_mar(
            self.get_export_file(self.get_name("max")))
