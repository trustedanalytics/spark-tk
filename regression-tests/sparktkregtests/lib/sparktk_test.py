...
def get_context():
    ...
    with lock:
        if global_tc is None:
                print ('RUNMODE = '), config.run_mode
                if config.run_mode:
                    #global_tc = stk.TkContext(master='yarn-client', extra_conf={'spark.driver.memory': "3712m", 'spark.executor.instances' : '5'})
                    global_tc = stk.TkContext(master='yarn-client', extra_conf={'spark.driver.memory': "3712m", 'spark.executor.instances' : '5', 'spark.driver.cores': '1', 'spark.driver.extraJavaOptions': '-Xmx2688m', 'spark.driver.maxPermSize': '512m', 'spark.driver.maxResultSize': '2g', 'spark.dynamicAllocation.enabled': 'true', 'spark.dynamicAllocation.maxExecutors': '116', 'spark.dynamicAllocation.minExecutors': '1', 'spark.executor.cores': '1', 'spark.executor.extrajavaoptions': '-Xmx2688m','spark.executor.memory': '3200m', 'spark.shuffle.io.preferDirectBufs': 'false', 'spark.shuffle.service.enabled': 'true', 'spark.yarn.am.waitTime': '1000000', 'spark.yarn.driver.memoryOverhead': '384', 'spark.yarn.executor.memoryOverhead': '384','spark.eventLog.enabled': 'false', 'spark.sql.shuffle.partitions': '6'})
