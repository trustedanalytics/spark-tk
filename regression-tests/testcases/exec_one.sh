...
echo "Call to nosetests" >> $RESULTS_DIR/run-one.log
echo "`date`" >> $RESULTS_DIR/run-one.log

echo "Kill any existing nosetests in progress"
pkill -f nosetests

time nosetests-2.7 -w $TESTS_DIR --tests=$SCRIPT -s -vv --with-time --processes=$NUM_PROCS --process-timeout=$PROC_TIMEOUT 1>> $RESULTS_DIR/run-one.log 2>> $RESULTS_DIR/run-one.err
...
RESULT_FILE=$RESULTS_DIR/nosetests-one.xml
...
if [[ $success == 0 ]] ; then
   echo "Python Tests Successful" >> $RESULTS_DIR/run-one.log
   echo "`date`" >> $RESULTS_DIR/run-one.log
   exit 0
fi
echo "Python Tests Unsuccessful" >> $RESULTS_DIR/run-one.log
echo "`date`" >> $RESULTS_DIR/run-one.log
exit 1
