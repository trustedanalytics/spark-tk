...
import org.trustedanalytics.sparktk.dicom.internal.constructors.Import.importDicom
...
class DicomHdfs extends TestingSparkContextWordSpec with Matchers {
  ...
  "dicom folder" should {
    "dicom files load and create dataframe test" in {
      ...
      val dicomFrame = importDicom(sparkContext, "hdfs://10.7.151.97:8020/user/kvadla/dicom_images_decompressed")
      val metadaFrame = dicomFrame.metadataFrame
      val imageFrame = dicomFrame.imagedataFrame

      println(metadaFrame.take(1)(0)(0) + ":" + metadaFrame.take(1)(0)(1))
      println("metadata count: " + metadaFrame.rowCount())
      println("imagedata count: " + imageFrame.rowCount())
