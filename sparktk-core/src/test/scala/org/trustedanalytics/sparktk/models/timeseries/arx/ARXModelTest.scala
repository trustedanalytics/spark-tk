...
class ArxModelTest extends TestingSparkContextWordSpec with Matchers {
  ...
  "train" should {
    "throw an exception for a null frame" in {
      intercept[IllegalArgumentException] {
        ...
        ArxModel.train(null, y_column, x_columns, 0, 0)
      }
    }
    ...
    "throw an exception for bad ts column name" in {
      ...
      intercept[IllegalArgumentException] {
        ...
        ArxModel.train(frame, "ts", x_columns, 0, 0)
      }
    }
    ...
    "throw an exception for bad x column name" in {
      ...
      intercept[IllegalArgumentException] {
        ...
        ArxModel.train(frame, y_column, List("CO_GT", "bogus", "NMHC_GT", "C6H6_GT"), 0, 0)
      }
    }
    ...
    "model can be trained with valid arguements" in {
      ...
      // Train
      val trainResult = ArxModel.train(frame, y_column, x_columns, 0, 0, true)
    }
  }
  ...
  "score" should {
    "return predictions when calling the ARX model score" in {
      val rdd = sparkContext.parallelize(rows)
      val frame = new Frame(rdd, schema)
      val model = ArxModel.train(frame, y_column, x_columns, 0, 0, true)
