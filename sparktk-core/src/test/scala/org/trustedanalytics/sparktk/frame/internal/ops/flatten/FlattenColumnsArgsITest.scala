...
class FlattenColumnArgsITest extends WordSpec with Matchers with BeforeAndAfterEach with TestingSparkContextWordSpec {
  "flattenRddByStringColumnIndex" should {
    ...
    "flatten a single string column" in {
    ...
    "flatten multiple string columns" in {
      val rows = List(Row("a,b,c", "10,18,20"), Row("d,e", "7,8"), Row("f,g,h", "17,2"))
    ...
    "flatten multiple columns with strings and vectors" in {
    ...
    "flatten multiple columns with nulls when the number of items in each column do not match" in {
...
class FlattenVectorColumnArgsITest extends WordSpec with Matchers with BeforeAndAfterEach with TestingSparkContextWordSpec {
  "flattenRddByVectorColumnIndex" should {
    "create separate rows when flattening entries" in {
    ...
    "flatten a string column and a vector column in the same RDD" in {
      val rows = List(
        Row("a,b,c", Vector[Double](10, 18, 20)),
