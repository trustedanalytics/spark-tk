 ...
 */
class CategoricalSummaryITest extends TestingSparkContextWordSpec with Matchers {
  ...
  "matchMissingValues" should {
    "return true for null" in {
      val value = (null, 1)
      CategoricalSummaryFunctions.matchMissingValues(value) shouldBe true
    }

    "return false for non null data" in {
      val value = ("valid", 1)
      CategoricalSummaryFunctions.matchMissingValues(value) shouldBe false
    }

    "return true for empty data" in {
      val value = ("", 1)
      CategoricalSummaryFunctions.matchMissingValues(value) shouldBe true
    }
  }

  "getTotalCountForSummaryLevels" should {
    "return 10 for total count of all categories" in {
      val levels = List(LevelData("a", 4, 0.4), LevelData("b", 3, 0.3), LevelData("c", 3, 0.3))
      CategoricalSummaryFunctions.getTotalCountForSummaryLevels(levels) shouldBe 10
    }
  }
