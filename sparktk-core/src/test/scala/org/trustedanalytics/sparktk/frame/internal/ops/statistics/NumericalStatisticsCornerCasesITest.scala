...
import org.trustedanalytics.sparktk.frame.internal.ops.statistics.numericalstatistics.NumericalStatistics
import org.trustedanalytics.sparktk.testutils.TestingSparkContextWordSpec

class NumericalStatisticsCornerCasesITest extends TestingSparkContextWordSpec with Matchers {
  ...
  "NumericalStatistics" should {

    "ignore values with non-positive weights, except for calculating count of non-positive weight data " in new NumericalStatisticsCornerCaseTest() {
    ...
    "ignore values with NaN or infinite weights, except for calculating count of bad rows " in new NumericalStatisticsCornerCaseTest() {
      val data = List(1, 2, 3, 4, 5, 6, 7, 8).map(x => x.toDouble)
      val frequencies = List(3, 2, 3, 1, 9, 4, 3, 1).map(x => x.toDouble)
    ...
    "give a NaN geometric mean when a data value is negative" in new NumericalStatisticsCornerCaseTest() {
    ...
    "give a NaN geometric mean when the data value is 0" in new NumericalStatisticsCornerCaseTest() {
    ...
    "support data of length 1" in new NumericalStatisticsCornerCaseTest() {
    ...
    "support data of length 2" in new NumericalStatisticsCornerCaseTest() {
    ...
    "support data of length 3" in new NumericalStatisticsCornerCaseTest() {
