...
object InferSchema {
  ...
  /**
    ...
    */
  def apply(tokenRdd: RDD[Example]): FrameSchema = {
    val featuresLength= tokenRdd.first().getFeatures.getFeatureCount

    val startType: Map[String, DataType] = Map.empty[String, DataType]
    val rootTypes: Map[String, DataType] = tokenRdd.aggregate(startType)(inferRowType, mergeRowTypes)
  ...
  def inferRowType(rowSoFar: Map[String, DataType], next: Example): Map[String, DataType] = {
    var i = 0
    next.getFeatures.getFeatureMap.asScala.map{ case(featureName, feature) =>{
