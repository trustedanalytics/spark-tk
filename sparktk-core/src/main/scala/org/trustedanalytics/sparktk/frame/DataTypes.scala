...
object DataTypes {
  ...
  lazy val logger = LoggerFactory.getLogger("DataTypes")
  /**
   * The datatype trait
   */
  ...
  private def toComplexDataType(s: String): DataType = {
    ...
    s match {
      ...
      case "_Vector" => DataTypes.string
      case "matrix" => DataTypes.matrix
      case "_Matrix" => DataTypes.matrix
  ...
  def toBigDecimalArray(value: Any, delimiter: String = ","): Array[BigDecimal] = {
    ...
    value match {
      ...
      case s: String => s.split(delimiter).map(x => {
        if (org.apache.commons.lang3.StringUtils.isBlank(x)) null.asInstanceOf[BigDecimal] else toBigDecimal(x)
      })
