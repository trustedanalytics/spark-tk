...
case class MaxModel private[max] (timeseriesColumn: String,
  ...
  override def score(data: Array[Any]): Array[Any] = {
    ...
    if (data.length == 0)
      throw new IllegalArgumentException("Unable to score using MAX model, because the array of data passed in is empty.")
  ...
  override def modelMetadata(): ModelMetaDataArgs = {
    new ModelMetaDataArgs("MAX Model", classOf[SparkTsMaxModel].getName, modelReader, Map())
  }
