...
case class ShortestPath(landmarks: Seq[Any]) extends GraphSummarization[DataFrame] {

  override def work(state: GraphState): DataFrame = {
    state.graphFrame.shortestPaths.landmarks(landmarks).run()
  }
