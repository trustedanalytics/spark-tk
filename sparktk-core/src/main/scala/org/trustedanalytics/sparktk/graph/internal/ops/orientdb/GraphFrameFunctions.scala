...
class GraphFrameFunctions(state: GraphState) {
   ...
   */
  def saveToOrientGraph(dbUrl: String, userName: String, password: String, rootPassword: String, vertexTypeColumnName: Option[String] = None, edgeTypeColumnName: Option[String] = None, batchSize: Option[Int] = Some(1000)): ExportToOrientdbReturn = {
    ...
    val verticesCount = vertexFrameWriter.exportVertexFrame(orientConf.batchSize.get, vertexTypeColumnName)
    val edgeFrameWriter = new EdgeFrameWriter(state.graphFrame.edges, orientConf)
    val edgesCount = edgeFrameWriter.exportEdgeFrame(orientConf.batchSize.get, edgeTypeColumnName)
    //collect statistics
