...
abstract class GradientWithFrequency extends Serializable {
   ...
   */
  def compute(data: Vector, label: Double, frequency: Double, weights: Vector): (Vector, Double) = {
    val gradient = Vectors.zeros(weights.size)
    val loss = compute(data, label, frequency, weights, gradient)
    (gradient, loss)
  }
   ...
   * @return loss
   */
  def compute(data: Vector, label: Double, frequency: Double, weights: Vector, cumGradient: Vector): Double
}
 ...
 *
 * The model weights w = (w_1, w_2, ..., w_{K-1}) becomes a matrix which has dimension of
 ...
 */
@DeveloperApi
class LogisticGradientWithFrequency(numClasses: Int) extends GradientWithFrequency {

  def this() = this(2)

  override def compute(data: Vector, label: Double, frequency: Double, weights: Vector): (Vector, Double) = {
    val gradient = Vectors.zeros(weights.size)
    val loss = compute(data, label, frequency, weights, gradient)
    (gradient, loss)
  }
  ...
  override def compute(
    ...
    numClasses match {
      case 2 =>
        ...
        val multiplier = frequency * ((1.0 / (1.0 + math.exp(margin))) - label)
        ...
        if (label > 0) {
          ...
          frequency * (MLUtils.log1pExp(margin) - margin)
        }
      case _ =>
        /**
        ...
        for (i <- 0 until numClasses - 1) {
          val multiplier = frequency * (math.exp(margins(i)) / (sum + 1.0) - {
 ...
 */
@DeveloperApi
class LeastSquaresGradientWithFrequency extends GradientWithFrequency {
  override def compute(data: Vector, label: Double, frequency: Double, weights: Vector): (Vector, Double) = {
