...
class CumulativeDistTest extends TestingSparkContextWordSpec with Matchers {
  ...
  "cumulative sum" should {
    "compute correct distribution" in {
    ...
    "compute correct distribution with different partitioning" in {
      for {
        i <- 1 to 10
      } {
    ...
    "compute correct distribution with over partitioning" in {
    ...
    "throw error for non-numeric columns" in {
      val rdd = sparkContext.parallelize(inputList)
      val frame = new FrameRdd(inputSchema, rdd)
      a[SparkException] shouldBe thrownBy(CumulativeDistFunctions.cumulativeSum(frame, "col_2"))
    }

    "compute correct distribution for column of all zero" in {
      val rdd = sparkContext.parallelize(inputList)
      val frame = new FrameRdd(inputSchema, rdd)
  ...
  "cumulative count" should {
    ...
    "compute correct distribution for column of strings" in {
      val rdd = sparkContext.parallelize(inputList)
      val frame = new FrameRdd(inputSchema, rdd)
  ...
  "cumulative percent count" should {
    "compute correct distribution" in {
      val rdd = sparkContext.parallelize(inputList)
      val frame = new FrameRdd(inputSchema, rdd)
  ...
  "paritionSums" should {
    "compute correct sum for one partition" in {
    ...
    "compute correct sums for two partitions" in {
    ...
    "compute correct sums for three partitions" in {
    ...
    "compute correct sums for seven partitions" in {
