...
def top_k(self, column_name, k, weight_column=None):
    ...
    :param column_name: The column whose top (or bottom) K distinct values are to be calculated.
    :param k: Number of entries to return (If k is negative, return bottom k).
    :param weight_column: The column that provides weights (frequencies) for the topK calculation.
    ...
    <hide>
    >>> frame = tc.to_frame([[1, "Portland", 609456, 583776, "4.40%", "Multnomah" ],[2, "Salem", 160614, 154637, "3.87%", "Marion" ], [3, "Eugene", 159190, 156185, "1.92%", "Lane" ],
    ...
    """
    return self._tc.to_frame(self._scala.topK(column_name, k, self._tc.jutils.convert.to_scala_option(weight_column)))